...
{
    private lateinit var gameManager: Manager
    ...
    */
    ...
    override fun onCreate(savedInstanceState: Bundle?)
    {
        ...
        setContentView(R.layout.activity_game)          //we specify another layout
        ...
        //Down we specify exact positions
    ...
    private fun resetboxes()                //resets all boxes, if u wanna play again
    {
        one.text = ""
        two.text = ""
        three.text = ""
    ...
    private fun onBoxClicked(box: TextView, position: Position) {                        //we cannot click 2 and more times on one box
        if (box.text.isEmpty()) {
            box.text = gameManager.currentPlayerMark
            val winningLine = gameManager.makeMove(position)
            if (winningLine != null) {                                                //if someone WINS....
                updatePoints()                                                       //we increase his points
                disableBoxes()                                                      //then we must disable all boxes
                startNewGameButton.visibility = View.VISIBLE                       //if someone wins, we cannot play anymore so start button shows up to start a new game
                showWinner(winningLine)
            }
            /**********************************************************************************************************/

            if ((one.text != "") && (two.text != "") && (three.text != "") && (four.text != "") && (five.text != "")
                &&  (six.text != "") && (seven.text != "") && (eight.text != "") && (nine.text != "") && (winningLine == null))
            {
                disableBoxes()
    ...
    private fun disableBoxes()                  //game ended and we don´t wanna continue to play
    {
        one.isEnabled = false
        two.isEnabled = false
    ...
    //all possible lines when there is winner
